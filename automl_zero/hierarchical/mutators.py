...
def _mutate_all(winner,gene_to_mutate, memory_dict_len, MAX_ARG=2):
    ...
    if gene_to_mutate == "gene_setup":
        genome_OPs = np.random.randint(LOW_SETUP_OPS,UNIQUE_SETUP_OPS-1, size=(genome_OPs.shape[0],genome_OPs.shape[1])).astype(np.int8)
        metalevel_gene = np.random.randint(0,METALEVEL_COUNT-1, size=(metalevel_gene.shape[0],metalevel_gene.shape[1])).astype(np.int8)

    elif gene_to_mutate == "gene_pred":
        genome_OPs = np.random.randint(LOW_PRED_OPS,UNIQUE_PRED_OPS-1, size=(genome_OPs.shape[0],genome_OPs.shape[1])).astype(np.int8)
        metalevel_gene = np.random.randint(0,METALEVEL_COUNT-1, size=(metalevel_gene.shape[0],metalevel_gene.shape[1])).astype(np.int8)

    else:
        genome_OPs = np.random.randint(LOW_LEARN_OPS,UNIQUE_LEARN_OPS-1, size=(genome_OPs.shape[0],genome_OPs.shape[1])).astype(np.int8)
        metalevel_gene = np.random.randint(0,METALEVEL_COUNT-1, size=(metalevel_gene.shape[0],metalevel_gene.shape[1])).astype(np.int8)
...
def _mutate_add_or_remove_one_instruction(winner, gene_to_mutate,  memory_dict_len, MAX_ARG=2):
    ...
    if gene_to_mutate == "gene_setup":
        metalevel_gene[instruction_idx:instruction_idx+1] = np.random.randint(0,METALEVEL_COUNT-1, size=(1)).astype(np.int8)
        if metalevel_gene[instruction_idx] == 0:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,UNIQUE_SETUP_OPS-1, size=(1)).astype(np.int8)
        else:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,NUMBER_OF_META_OPS-1, size=(1)).astype(np.int8)
        

    elif gene_to_mutate == "gene_pred":
        metalevel_gene[instruction_idx:instruction_idx+1] = np.random.randint(0,METALEVEL_COUNT-1, size=(1)).astype(np.int8)
        if metalevel_gene[instruction_idx] == 0:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,UNIQUE_PRED_OPS-1, size=(1)).astype(np.int8)
        else:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,NUMBER_OF_META_OPS-1, size=(1)).astype(np.int8)
        ...
        metalevel_gene[instruction_idx:instruction_idx+1] = np.random.randint(0,METALEVEL_COUNT-1, size=(1)).astype(np.int8)
        if metalevel_gene[instruction_idx] == 0:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,UNIQUE_LEARN_OPS-1, size=(1)).astype(np.int8)
        else:
            genome_OPs[instruction_idx:instruction_idx+1] = np.random.randint(0,NUMBER_OF_META_OPS-1, size=(1)).astype(np.int8)
...
def mutate_combination_single(combine_OPs, NUMBER_OF_BASE_OPS = None):
    ...
    metalevel_idx = np.randint(0, METALEVEL_COUNT-1, size=(1))[0]
    op_idx = np.randint(0, NUMBER_OF_OPS-1, size=(1))[0]
    new_op_idx = np.randint(0, PRIOR_LEVEL_OPS-1, size=(1))[0]
    new_op_value = np.randint(0, NUMBER_OF_OPS-1, size=(1))[0]

    if metalevel_idx==0:
        new_op_value = np.randint(0, NUMBER_OF_BASE_OPS-1, size=(1))[0]
    else:
        new_op_value = np.randint(0, NUMBER_OF_OPS-1, size=(1))[0]
...
def mutate_combination_all_ops(combine_OPs, NUMBER_OF_BASE_OPS = None):
    ...
    op_idx = np.randint(0, NUMBER_OF_OPS-1, size=(1))[0]
    if metalevel_idx==0:
        new_ops = np.randint(0, NUMBER_OF_BASE_OPS-1, size=(PRIOR_LEVEL_OPS))[0]
    else:
        new_ops = np.randint(0, NUMBER_OF_OPS-1, size=(PRIOR_LEVEL_OPS))[0]
...
def mutate_combination_all(combine_OPs, NUMBER_OF_BASE_OPS = None):
    ...
    combine_OPs[0,:,:] = np.random.randint(0, NUMBER_OF_BASE_OPS-1 , size=(1, NUMBER_OF_META_OPS,PRIOR_LEVEL_OPS )).astype(np.float64)
